...
async function Draw(){
  ...
  if(!t){
    ...
    subbed = (subs, size, sphereize, shape) => {
      ...
      if(sphereize){
        ...
        shape.map((v,i)=>{
          ...
          if(!cull && (subs==1 || v.length>3 || i<shape.length-0)) newshape = [...newshape, a]
        })
        shape = newshape
      }
      return shape
    ...
    ls = 5, sp = 9.5
    for(j=sd2;j--;){
      X = S(p=Math.PI*2/sd2*j+Math.PI/sd2/2)*ls
      Y = C(p)*ls
      Z = 0
      a = [...a, [X, Y, Z]]
    }
    ...
    a = []
    for(i=sd;i--;){
      tx = S(p=Math.PI*2/sd*i+Math.PI/sd/2) * sp
      ty = C(p) * sp
      tz = 0
      a = [...a, ...JSON.parse(JSON.stringify(b)).map((v,j)=>{
        v.map(q=>{
          X = q[0]
          Y = q[1]
          Z = q[2]
          R(Math.PI*2/sd*i,0,0)
          q[0] = X + tx
          q[1] = Y + ty
          q[2] = Z + tz
        })
    ...
    base_shp = [...base_shp, ...JSON.parse(JSON.stringify(base_shp)).map(v=>{
      v.map(q=>{
        X = q[0]
        Y = q[1]
        Z = q[2]
    ...
    spawnB = () => {
      ...
      X = Y = Z = 0
      for(m=shockLen;m--;){
        vx += (Rn()-.5) * squirreliness
        vy += (Rn()-.5) * squirreliness / 4
        vz += (Rn()-.5) * squirreliness
        vx /= dampening
        vy /= 1+dampening/40
        vz /= dampening
        a = [...a, [X, Y, Z, vx, vy, vz]]
  ...
  oX=0, oY=0, oZ=Math.min(35,Math.max(10,(.3+C(t/4))*80))
  Rl=0, Pt=S(t/3)*Math.PI/2, Yw = -S(t/5)*4
  ...
  base_shp.map((v,i)=>{
    if(1){
      ...
      alpha = .15-C(t)/2
      //stroke(alpha<.66?`hsla(${160},99%,50%,${(.66-alpha)/1.5*.5})`:'',alpha>=.33?`hsla(${250},99%,50%,${(.66-(1-alpha))/1.5*.5})`:'',4,true)
      stroke(alpha<.5?`hsla(${160},99%,50%,${(.5-alpha)/1.5*.5})`:'',alpha>=.33?`hsla(${250},99%,50%,${(.5-(1-alpha))/1.5})`:'',3,false)
    }
  })
  ...
  B.map((v,i)=>{
    v[0].map((q, j)=>{
      if(j){
        ...
        if(Z>0)x.lineTo(...Q())
        alpha = .66*v[1]
        stroke(`hsla(${j*10+t*20000+360/B.length*i},99%,${100-j*5}%,${alpha})`,'',10/(1+j/4),true)
      }
    })
    v[1] -= .05
  })
  x.lineCap = x.lineJoin = tlineCap
  ...
  if(!((t*60|0)%3)){
    ...
    if(Z>0){
      l = Q()
      s = Math.min(1e4,3e4/Z)
      x.drawImage(starImgs[1].img,l[0]-s/2,l[1]-s/2,s,s)
      s/=2
      x.drawImage(starImgs[2+Rn()*7|0].img,l[0]-s/2.05,l[1]-s/2.05,s,s)
    }
  }
